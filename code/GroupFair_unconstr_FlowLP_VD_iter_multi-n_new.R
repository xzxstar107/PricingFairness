...
for (n1 in nrange) {
  ...
  f.con <- mat
  f.dir <- c(rep("<=",k*l), rep("<=", n), rep("=",k), rep(">=", l*n + k*l+k*(k-1)*l/2))
  f.rhs <- c(rep(0,k*l), rep(1, n), rep(1,k), rep(0, l*n + k*l+k*(k-1)*l/2))
...
write.csv(params_init, file = paste0('paramsSim_unconst_n.csv'))
write.csv(opt_res, file = paste0('res_unconst_n_',paramind,'.csv'))
