...
for (n1 in nrange) {
  ...
  # obj_vec <- lp ("max", f.obj, f.con, f.dir, f.rhs)$objective
  # var_num <- lp ("max", f.obj, f.con, f.dir, f.rhs)$x.count
  # constr_num <- lp ("max", f.obj, f.con, f.dir, f.rhs)$const.count
  # constr_mat <- lp ("max", f.obj, f.con, f.dir, f.rhs)$constraints
  # tail(opt_sol, k*l+k*(k-1)*l/2)
  ...
  opt_res[ind,] <- c(n1, n2, opt_obj, opt_obj1,opt_obj2, opt_surplus,opt_surplus1,opt_surplus2, 
                     opt_welfare,opt_welfare1,opt_welfare2, tail(opt_sol, k*l))
}

# Output settings and results
write.csv(params_init, file = paste0('paramsSim_TV_n_new.csv'))
write.csv(opt_res, file = paste0('res_TV_n_',paramind,'_new.csv'))
